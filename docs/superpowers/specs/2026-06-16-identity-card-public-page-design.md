...
**~~Deferral (2)~~ → implemented:** the spec's `AccountRow` platform icon was initially dropped by the plan's Task 6, then **added on owner request** during verification. Each row now shows the **platform name** (`Threads`) + a monochrome **brand glyph** from [Simple Icons](https://simpleicons.org) (SVG path data is **CC0/public-domain**; rendered via `currentColor` and treated as decorative `aria-hidden`, with the text label carrying the accessible name). New `PlatformIcon.tsx`; Threads + Instagram glyphs bundled (Instagram for the future adapter), unknown platforms degrade to label-only. Nominative use (identifying the platform an account lives on) — no endorsement implied.
