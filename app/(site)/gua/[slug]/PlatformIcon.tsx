...
// Per-platform BRAND treatment so platforms are distinguishable at a glance. A colorful brand
// renders in its brand color/gradient; a monochrome brand (Threads) has no entry and inherits
// `currentColor`. WHEN ADDING A PLATFORM: register its glyph in PATHS above, and — if its brand is
// colorful — its treatment here. (Doc: docs/product-decisions.md "Platform icon brand identity".)
...
const BRAND: Record<string, Brand> = {
  ...
  // miin: glyph + brand TBD (no reliable mark yet) → renders nothing until registered.
};
...
export function PlatformIcon({
  ...
  const gradId = `platicon-${useId().replace(/:/g, "")}`;
