...
- **Stale-rid hardening (all platforms):** visiting `/add/{platform}?rid=<x>` where the request is non-existent / another user's / wrong-platform / expired / resolved / verified / cancelled now **redirects to a clean `/add/{platform}`** (preserving `?recover=`) instead of silently rendering the produce-template screen with the dead token in the URL. Extracted as **`loadWizardRequest()`** — `/add/[platform]` is one dynamic route, so this single rule covers Threads + miin + future platforms. Every not-live case lands identically (no rid-status enumeration oracle); the confirm page already had the analogous redirect.
