...
- **登出 / 切換帳號 promoted to a global account menu:** new **`AccountMenu`** client dropdown in the top-right of the global `SiteHeader` — an **avatar** trigger opens 正身頁面 / 切換帳號 / 登出 (outside-click + Esc close). Reachable on every page, replacing the bottom-of-manage-view links (which required owner → manage → scroll). The two actions moved to a shared **`app/account-actions.ts`**; the card's `id-manage-links` block is gone, and the logged-out header still shows 登入 / 免費註冊.
