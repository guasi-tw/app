...
- **Header 登入 / 免費註冊 → Google directly:** both buttons now POST a shared `googleSignInAction` (`signIn("google", { redirectTo: "/post-login" })`) instead of linking to `/login`, removing one hop to the OAuth consent screen. The primary CTA is relabeled **「使用 Google 免費註冊」** so the copy matches the destination (Google-only MVP). `/login` stays as the fallback (protected-route redirects + a future email/multi-method picker). Buttons render via `<form>` with `.site-link`/`.site-cta` button resets.
