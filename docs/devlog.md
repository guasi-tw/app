...
- **Icons across the add flow:** the `/add` platform picker tiles (active + 施工中) and the `/add/{platform}` page headings now show the platform glyph. Because `PlatformIcon` is shared, the **Accounts tab** picks up the brand colors too.
