...
- Scoped re-verify: `?recover={accountId}` threaded through `/add/{platform}` → wizard → `/add/{platform}/confirm`, with a same-account guard ("這則貼文不是這個帳號發的") before `reverifyBinding` refreshes the proof. The add page is recovery-aware (heading 重新驗證, lede names the target `@handle` resolved from the accountId, button 產生重新驗證貼文); a `recover` that doesn't resolve to one of the caller's own bindings is refused up front ("找不到要恢復的帳號") rather than dead-ending at the commit guard.
