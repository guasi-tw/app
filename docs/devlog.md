...
| Version | Summary |
|---------|---------|
| [v0.14.2](#v0142--about-platform-independence-section-2026-06-18) | **About page — platform-independence feature.** Surfaced *platform independence* as a first-class differentiator on `/about` (previously buried in one defensive trust bullet). New emphasized **平台中立 · 不綁任何一家平台** section (gold-styled, placed right after the platform strip) reframes 「更多陸續支援」 as a consequence of the architecture: guasi needs no platform API/authorization — verification rides on **the user's own public post**, so any platform with public posts is supportable and the banning platform can't gate your 正身. Also tightened the existing trust bullet `不靠平台授權` → **`不靠平台授權，只靠公開貼文`** (mechanism-first) so the two reinforce rather than repeat. Copy-only (typed `content.ts` + CSS-module reuse, no new styles); +1 accuracy test (144 tests). |
...
- +1 accuracy-constraint test locking the new copy (title `不綁`, body mentions `公開貼文` + `授權`). 144 tests.
- Copy-only: typed `content.ts` (new `independence` block + reworded trust item), `page.tsx` (one section), `content.test.ts`. No logic, schema, or style changes.

**Key technical learnings:**
- `[insight]` The platform-independence claim already existed on the page but was *buried and framed defensively* (a single trust bullet about not using OAuth). Promoting it to a standalone, positively-framed section + leading the trust bullet with the mechanism made a feature already-true in the product legible to readers — a copy/placement change, not a product change.
- `[note]` Built in an isolated worktree (`about-platform-independent`, off `main`) to stay clear of the parallel Slice 4 timeline session working in the main tree; released as `v0.14.2` on the shipped `v0.14.x` line so `v0.15.0` stays earmarked for the timeline tab.
