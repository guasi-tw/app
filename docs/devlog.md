...
- Scoped re-verify: `?recover={accountId}` threaded through `/add/{platform}` → wizard → `/add/{platform}/confirm`, with a same-account guard ("這則貼文不是這個帳號發的") before `reverifyBinding` refreshes the proof.
...
- `onboardedAt` post-login routing: slug-less-but-onboarded returning user → `/r/{shortRef}`; genuine first-timer → `/onboarding`; stamped once on first onboarding completion.
