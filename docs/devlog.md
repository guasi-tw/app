...
| Version | Summary |
|---------|---------|
| [v0.17.0](#v0170--manage-view-ux-refinements-2026-06-18) | **Manage-view UX refinements** (`/gua/{slug}` IdentityCard). (1) **編輯個人資料** moved into the header **directly under the bio** (manage view), and (2) **＋註冊分身** moved to the **top of the 帳號 list, above the main account** — both out of the old bottom link stack (which now holds just 登出/切換帳號). (3) Switching **公開 ⇄ 管理** always **returns to the 帳號 tab** (`selectMode` resets `tab`). (4) The public/manage control is now an **on/off switch** (`管理模式`, `role="switch"`) instead of a two-button segmented control that read like tabs. (5) Private rows in the **manage-view timeline** get a **subtle gray wash** (owner-only), alongside the existing hollow dot + `👁 私密` tag. (6) **Share CTA surfaced** from the buried footer to a card **under the profile, above the tabs**, with the caption **「分享正身連結，讓大家能驗明正身」**. CSS-only `.id-switch`/`.id-add-account`/`.id-share`/`.tl-item.priv`; 176 tests (unchanged). |
...
- **Share CTA surfaced:** the `複製連結` `ShareLink` moved from the buried footer to a **share card under the profile, above the tabs** (`.id-share`), with an encouraging caption **「分享正身連結，讓大家能驗明正身」** (the `驗明正身` wordplay). Shown to everyone viewing a public 正身 (`publicUrl`-gated); the `guasi.tw/gua/{slug}` link is the cross-platform profile meant to be pasted into other bios.
