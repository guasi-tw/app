...
- **Sign-in CTAs start Google directly (app-wide):** every user-facing 登入/註冊 CTA now POSTs a shared **`googleSignInAction`** (`signIn("google", { redirectTo: "/post-login" })`) via a reusable **`GoogleSignInButton`**, instead of linking to `/login` — removing one hop to the OAuth consent screen. Applied to the **header** (登入 kept compact; primary relabeled **「使用 Google 免費註冊」** so copy matches a Google-only destination), the **home** CTA, the **/about** CTA (dropped the now-unused `cta.href`), and the **public card footer** (logged-out visitor). `/login` stays as the fallback (protected-route redirects + a future email/multi-method picker). Button-as-link resets via `.btn-as-link` + per-class resets.
